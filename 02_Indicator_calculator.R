...
perc_world <- read.csv('data/percent_protected_world.csv')
landlocked <- read.csv('data/landlocked.csv') %>% 
  janitor::clean_names()

landlocked <- landlocked$i_country
...
# I will summarise by SOVEREINITY, since it represents better the authority on the area
