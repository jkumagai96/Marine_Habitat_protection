...
eez_land_global <- left_join(x = eez_land, y = data, by = "UNION") %>%  # Join the indicator data onto the eez_land 
        arrange(GPHPI_average)



# Bar plot for the top and bottom 10 
data1 <- data %>% 
        slice_max(order_by = Target_analysis_average, n = 10) 

data2 <- data %>% 
        slice_min(order_by = Target_analysis_average, n = 10)
...
data %>% 
        ggplot(aes(x = reorder(ISO_TER1, Target_analysis_average), y = Target_analysis_average)) +
        geom_bar(stat = 'identity', aes(fill = Target_analysis_average > 0), position = 'dodge', col = 'transparent') +
        theme_bw() +
        scale_fill_manual(guide = 'none',
                          values = c("red3", "#0868ac")) +
        labs(x = "Jurisdictions", y = "Targeted Global Proportion of Habitats Protected") +
        ylim(-max(abs(data$Target_analysis_average)), max(abs(data$Target_analysis_average))) +
        theme(axis.text.x = element_text(angle = 90, vjust = .5))


ggsave(last_plot(), filename = "Figures/figure4.png", dpi = 600, height = 6, width = 8)
ggsave(last_plot(), filename = "Figures/figure4.pdf", device = "pdf", height = 6, width = 8)
